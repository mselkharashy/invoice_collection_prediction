...
# Make a list
model_list <- list(
 glmnet = load('model_glmnet.rda'),
 rf = load('model_rf.rda'),
 gbm = load('model_gbm.rda'),
 svm = load('model_svm.rda')
 )
...
```{r}
varImp(model_list[['model_glmnet']])
```

# Prediction Result

```{r}
p_svm <- predict(model_list[['model_svm']], tst_data)
p_glmnet <- predict(model_list[['model_glmnet']], tst_data)
p_rf <- predict(model_list[['model_rf']], tst_data)
p_gbm <- predict(model_list[['model_gbm']], tst_data)
